...
| `streamingSubmitMode` | `"steer" \| "followUp"` | `"steer"` | What submitting a message does while the agent is streaming: `"steer"` interrupts the current turn with the message, `"followUp"` queues it until the turn ends. |
