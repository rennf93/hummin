...
describe("InteractiveThemeController", () => {
	...
	it("applies the fork default theme without querying twice", async () => {
		...
		// fork: an unset theme setting resolves to hummin-dark, so terminal
		// auto-detection is bypassed; the default-color query still runs once.
		expect(theme.name).toBe("hummin-dark");
		expect(queryTerminalBackgroundColor).toHaveBeenCalledOnce();
	});
