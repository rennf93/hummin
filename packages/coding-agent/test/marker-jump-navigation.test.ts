...
describe("findMarkerJumpTarget", () => {
	...
	it("jumps to largest contentRow strictly below scrollTop + 1 for previous", () => {
		expect(findMarkerJumpTarget(markers(0, 10, 20, 30), 21, "previous")?.contentRow).toBe(20);
		// scrollTop exactly on a marker row: per spec, previous is < scrollTop + 1,
		// so the row at scrollTop itself is a valid target
		expect(findMarkerJumpTarget(markers(0, 10, 20, 30), 20, "previous")?.contentRow).toBe(20);
	});

	it("jumps to smallest contentRow strictly above scrollTop for next", () => {
		expect(findMarkerJumpTarget(markers(0, 10, 20, 30), 10, "next")?.contentRow).toBe(20);
		expect(findMarkerJumpTarget(markers(0, 10, 20, 30), 11, "next")?.contentRow).toBe(20);
	});
	...
	it("handles unsorted marker arrays", () => {
		expect(findMarkerJumpTarget(markers(30, 0, 20, 10), 15, "previous")?.contentRow).toBe(10);
		expect(findMarkerJumpTarget(markers(30, 0, 20, 10), 15, "next")?.contentRow).toBe(20);
	});
});
