...
async function loadExtensionModule(extensionPath: string, cacheToken?: ExtensionCacheToken) {
	...
	// hummin: content-addressed transform cache - wrap jiti's default transform so
	// a hit skips babel entirely. The miss path calls jiti's own bundled babel
	// module (the same function jiti uses by default), so output is byte-identical
	// to uncached behavior. See transform-cache.ts; HUMMIN_JITI_CACHE=0 disables.
	...
	if (!isTransformCacheDisabled()) {
		...
		const cached = createTransformCache(agentDir, {
			...
			createBaseJiti: (opts) => createJitiImpl(import.meta.url, opts as JitiOptions),
		});
		jitiOptions = { ...baseOptions, transform: cached.transform };
		scheduleTransformCacheGc(agentDir);
	}

	const jiti = createJitiImpl(import.meta.url, jitiOptions);
