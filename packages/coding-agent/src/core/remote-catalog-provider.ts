...
export function withRemoteCatalog(
	...
	// fork: the shipped static catalog is the source of truth, so the default
	// pi.dev refresh is disabled. An explicit non-default catalogBaseUrl
	// (tests, custom deployments) opts back in.
	if (!REMOTE_CATALOG_ENABLED && catalogBaseUrl === DEFAULT_CATALOG_BASE_URL) {
		return provider;
	}
